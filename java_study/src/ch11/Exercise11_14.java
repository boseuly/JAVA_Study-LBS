...
public class Exercise11_14 {
	...
	public static void main(String[] args) {
		/*
		 [11-14] 다음은 성적처리 프로그램의 일부이다 클래스를 Scanner이용해서 화면으로부터 데이터를 입력하고 보여주는 기능을 완성하시오.
		 */
		while(true) {
			switch(displayMenu()) {
			case 1 :
	...
	static int displayMenu(){
		...
		*/
		while(true) {
			...
			menu = s.nextInt();		// 사용자가 입력한 값을 pick에 저장한다. 
			if(!(menu > 0 && menu <= 3)) {	// 만약 입력값이 범위를 벗어났다면
				System.out.println("메뉴를 잘못 선택하였습니다. 다시 입력하세요.");
				continue;	// 다시 사용자에게 입력 받기
			}
			break;
		}
		return menu;
		...
		static void inputRecord() {
			...
			while(true) {
					System.out.print(">>");
				/*
				(2) 아래의 로직에 맞게 코드를 작성하시오.							// 틀림
				1. Scanner를 이용해서 화면으로부터 데이터를 입력받는다. (',' 를 구분자로)
				2. 입력받은 값이 q또는 Q이면 메서드를 종료하고,
				 	그렇지 않으면 입력받은 값으로 Student9인스턴스를 생성하고 record에 추가한다.
				...
				*/
				...
				Student9 Student9 = (Student9)record.get(i);
				System.out.println(Student9);
				koreanTotal += Student9.kor;
				mathTotal += Student9.math;
				englishTotal += Student9.eng;
				total += Student9.total;
				}
				System.out.println("====================================================");
